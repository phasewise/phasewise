...
4. ✅ **Invoice header industry-standard gaps** (Stage 1 + most of Stage 2 shipped 2026-05-04) — **Stage 1**: Schema additions to `Organization` (billingMailingAddress, billingFedId, billingAchRouting/Account, billingWireRouting/Account). `/settings/billing-info` page. PDF + public viewer render a "Please Remit Payment Via" block. **Stage 2 partial**: `Project.contractNumber` added to schema; surfaced on project create + edit forms; renders as "Agreement No. {value}" in the invoice PDF and on the public viewer under the project block. **Still deferred**: per-client `Attn:` contact name surfacing on BILL TO, "Submit invoice in triplicate" public-agency toggle.
