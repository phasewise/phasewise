...
4. ✅ **Invoice header industry-standard gaps** (Stage 1 shipped 2026-05-04, Stage 2 deferred) — Schema additions to `Organization`: `billingMailingAddress`, `billingFedId`, `billingAchRouting`, `billingAchAccount`, `billingWireRouting`, `billingWireAccount`. All optional/nullable. New `/settings/billing-info` page with three sections (Firm Identity, ACH, Wire). API extended to GET/PATCH the new fields. PDF renderer renders a "Please Remit Payment Via" block under the header — Mail / ACH / Wire / Fed ID columns, only sections with data shown. Public `/invoice/[token]` page renders the same block. **Stage 2 deferred** (still in wishlist): per-project `Project.contractNumber`, per-client `Attn:` contact name surfacing on BILL TO, and the optional "Submit invoice in triplicate" public-agency toggle.
