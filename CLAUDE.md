...
3. ✅ **Auto-invoicing UX surfacing** (partial — shipped 2026-05-04) — Status panel added to top of `/admin/billing`: "Auto-invoicing — drafts every month for projects with billable hours. Next run: {date} · {N} drafts awaiting review" with "Review drafts →" CTA scrolling to the Draft section (which now has `id="draft-section"`). Quick-period preset buttons added above Period start/end on the New Invoice form: **Last month** / **This month** / **Last week** / **Clear**. **Still pending** for full closure: per-project billing cadence dropdown (`Project.billingCadence` enum), dashboard tile for drafts-to-review on the main `/dashboard` page.
