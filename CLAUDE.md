...
- **Optional: per-firm "Skip printing bank details on invoice" toggle (Kevin asked 2026-05-05)** — current behavior renders mailing address + ACH + Wire on every invoice (industry standard for B2B). But anyone with a copy of the invoice has the bank info. Add `Organization.printPaymentDetailsOnInvoice Boolean @default(true)` so firms can opt out — invoice will just say "Pay via [public link]" instead. Becomes more useful once Stripe Payment Links are live (the "Pay now" button is the privacy-preserving alternative to printing routing/account). Until then, ACH-only is a reasonable middle-ground (cheaper for clients, no card-fee skim).
...
- ✅ **Monthly leave accrual with cap + rollover** (org-level shipped 2026-05-04, per-user override deferred) — `LeavePolicyEntry` extended with `mode` ("FRONTLOAD" | "ACCRUED"), `monthlyAccrual`, and `cap` fields. New `computeAccruedHours(monthlyAccrual, cap, date)` helper counts COMPLETED months (Jan 15 yields 0 if Jan isn't done; really uses `date.getMonth()` which returns 0=Jan). `computeUserLeaveBalances` now returns the resolved mode + accrued amount alongside the existing fields, so the timesheet balance widget and STAFF dashboard automatically reflect the right number based on each leave type's mode. Admin form at `/admin/leave` exposes a Mode dropdown (Front-load default) plus Monthly accrual + Cap inputs (disabled when mode=Front-load). No cron needed — accrual is computed on read, not stored. **Per-user override editor** in the same admin page still uses the legacy 2-field shape (annualHours + rolloverCap) — extending it for accrual is a follow-up. Year-end rollover clamping is still manual / deferred.
